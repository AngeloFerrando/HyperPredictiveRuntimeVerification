...
{ int tmp;
  ...
  char thread_log_filename[20];
  FILE* thread_log;
  thread_log = fopen ("log5_2", "a");
  fprintf (thread_log,"<trace>\n<string key=\"concept:name\" value=\"3\"/>\n");
  fprintf (thread_log,"<string key=\"creator\" value=\"GD\"/>\n");
  char event[30];
    
  printEvent(thread_log,"Thread2","create_lock(M:1)");
  for(int i=0;i<k;i++) {
      //c=c+10
      printEvent(thread_log,"Thread2","lock(M)");
      sprintf(event, "read_shared_data(c%d)",i);
      printEvent(thread_log,"Thread2",event);
      sprintf(event, "write_shared_data(c%d)",i);
      printEvent(thread_log,"Thread2",event);
      printEvent(thread_log,"Thread2","unlock(M)");
    }
...
int main(int argc, char* argv[])
{
    ...
    rt2=pthread_create(&t2, NULL, fnC2, val);
