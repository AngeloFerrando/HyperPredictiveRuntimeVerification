...
def main(argv):
    ...
    for log in args.logs.split(','):
        ...
        # pm4py.save_vis_petri_net(net, initial_marking, final_marking, "petri.png")
        # return
                        ...
                        if a.startswith('(p)'):
                            ...
                            for t in transitions:
                                if t.name == b.replace('(t)', ''):
                                    new_transition = t
                                    break
                                if t.name.startswith('stop') and b.replace('(t)', '').startswith('stop'):
                                    stop = True
                                    break
                            if stop:
                                continue
                            if not new_transition:
                                ...
                                # elif b.replace('(t)', '').startswith('stop'):
                                #     new_transition = PetriNet.Transition('stop', None)
                                else:
                            ...
                            for t in transitions:
                                if t.name == a.replace('(t)', ''):
                                    new_transition = t
                                    break
                                if t.name.startswith('stop') and a.replace('(t)', '').startswith('stop'):
                                    stop = True
                                    break
                            if stop:
                                continue
                            if not new_transition:
                                ...
                                # elif a.replace('(t)', '').startswith('stop'):
                                #     new_transition = PetriNet.Transition('stop', None)
                                else:
    ...
    for p in new_net.places:
        if 'source' in p.name and 'final_source' not in p.name:
            petri_utils.add_arc_from_to(final_source_transition, p, new_net)
        elif 'sink' in p.name and 'final_sink' not in p.name:
    ...
    print('### Generation time:', (end-start).total_seconds(), 'seconds')
    start = datetime.datetime.now()

    # for t in new_net.transitions:
    #     print(t.name)
    ts = reachability_graph.construct_reachability_graph(new_net, initial_marking)
    print('Reachability graph generated')
        ...
        while True:
            ...
            for (c, id) in critical_sections:
                if c == state.name[j+9:k] and id != state.name[w+9:-1]:
                    ...
                    print('### Verification time:', (end-start).total_seconds(), 'seconds')
